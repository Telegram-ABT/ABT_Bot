...
async def handle_incoming_message(event):
    ...
    if record.get("firstcall", False):
        ...
        promt_text = (
            f"{promt}\n\n"
            "Проанализируй информацию о диалогах пользователя ниже. "
            f"Пользователь общается в группе '{record.get('chat_name', '')}', описание группы: {chat_discr}. "
            "Диалоги упорядочены в обратном хронологическом порядке (сначала старые, затем новые).\n\n"
            "Тебе нужно:\n"
            "1. Продолжить диалог в стиле сообщений пользователя, как если бы ты сам общался с ним.\n"
            "2. Сформулируй ответ в поддержку беседы и хода обсуждения, без лишнего приветствия.\n"
            "3. Используй тот же язык, на котором пишет собеседник.\n\n"
            "Примечание: если пользователь высказывает нежелание продолжать диалог или отвечает негативно, "
